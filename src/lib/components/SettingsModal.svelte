<script lang="ts">
	...
	let tab = $state<'providers' | 'tools' | 'skills' | 'generation' | 'appearance'>('providers');
	...
	>
		...
		>
			...
			<div class="flex flex-wrap gap-1 border-b border-line px-4 py-2 text-sm">
				{#each [['providers', 'Providers', 'key'], ['tools', 'Tools', 'globe'], ['skills', 'Skills', 'book'], ['generation', 'Generation', 'sliders'], ['appearance', 'Appearance', 'sparkles']] as [key, label, icon] (key)}
					<button
