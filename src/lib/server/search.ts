...
/** How long one search request may take, from the call to the last byte. */
...
export async function searxngSearch(
  ...
  if (!base) {
    throw new Error(
      "No SearXNG instance configured. Open Settings > Tools and set the SearXNG URL.",
    );
  }
  let endpoint: URL;
  try {
  ...
  // An instance that accepts the connection and then says nothing would otherwise
  // hold the tool call open for as long as the turn lives.
  const limit = signal
    ? AbortSignal.any([signal, AbortSignal.timeout(SEARCH_TIMEOUT_MS)])
    : AbortSignal.timeout(SEARCH_TIMEOUT_MS);
  let res: Response;
  try {
    res = await fetch(endpoint, { headers, signal: limit });
  } catch (err) {
    // A reader who stopped the turn is not a failure to explain twice.
    if (signal?.aborted) throw err;
    throw new Error(
      `The search instance did not answer in ${SEARCH_TIMEOUT_MS / 1000} s: ${base}`,
    );
  }
  if (!res.ok) {
  ...
  if (usage && usage.source === "json" && Array.isArray(body.error)) {
    throw new Error(
      `SearXNG rate limited: ${(body.error as unknown[]).join(" ")}`,
    );
  }
  const engines = Array.isArray(body.unresponsive_engines)
    ? body.unresponsive_engines.map(String)
    : undefined;
  return {
    query,
    provider: "SearXNG",
