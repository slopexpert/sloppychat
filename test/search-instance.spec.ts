...
describe('a search instance that will not answer', () => {
	it('says the instance was too slow when its own limit ends the request', async () => {
		vi.stubGlobal(
			...
			vi.fn(async (_url: string, init: { signal?: AbortSignal }) => {
				...
				expect(init.signal, 'the request carries a limit of its own').toBeDefined();
				throw error;
			})
		);

		await expect(searxngSearch(config, 'pi programming', 5)).rejects.toThrow(/did not answer in 15 s/);
	});
	...
	it('passes a stopped turn through untouched', async () => {
		...
		await expect(searxngSearch(config, 'pi programming', 5, caller.signal)).rejects.toBe(error);
	});

	it('tells the reader what an instance that answers badly means', async () => {
		vi.stubGlobal('fetch', vi.fn(async () => new Response('json format is disabled', { status: 403 })));

		await expect(searxngSearch(config, 'pi programming', 5)).rejects.toThrow(/HTTP 403.*search\.formats/);
	});
});
